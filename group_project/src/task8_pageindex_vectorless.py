...
try:
    from .rag_utils import default_chunks, legal_reference_match_score, tokenize
except ImportError:
    from rag_utils import default_chunks, legal_reference_match_score, tokenize
...
def pageindex_search(query: str, top_k: int = 5) -> list[dict]:
    ...
    for doc in _load_pageindex_docs():
        ...
        # Preserve exact legal-reference behavior even when hybrid retrieval
        # falls back to the local vectorless index.
        score += legal_reference_match_score(query, doc.get("metadata", {}), doc["content"])
        if score <= 0:
            continue
        results.append(
            {
