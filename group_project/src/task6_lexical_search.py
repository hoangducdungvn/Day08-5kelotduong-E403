...
try:
    from .rag_utils import default_chunks, extract_legal_references, legal_reference_match_score, tokenize
except ImportError:
    from rag_utils import default_chunks, extract_legal_references, legal_reference_match_score, tokenize
...
def lexical_search(query: str, top_k: int = 5) -> list[dict]:
    ...
    # Exact legal references are high-precision intent. Boosting them in BM25
    # prevents semantic near-matches from burying the requested Điều/Khoản.
    query_refs = extract_legal_references(query)
    if any(query_refs.values()):
        for index, document in enumerate(corpus):
            metadata = document.get("metadata", {})
            reference_score = legal_reference_match_score(query, metadata, document.get("content", ""))
            if reference_score:
                scores[index] += 12.0 * reference_score
